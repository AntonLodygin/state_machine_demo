...
void VEPolyline::hoverEnterEvent(QGraphicsSceneHoverEvent *event)
{
    ...
    for(int i = 0; i < linePath.elementCount(); i++){
        ...
        Grabber *dot = new Grabber(point, this);
        connect(dot, &Grabber::signalMove, this, &VEPolyline::slotMove);
        connect(dot, &Grabber::signalMouseRelease, this, &VEPolyline::checkForDeletePoints);
        dot->setGrabberFlags(Grabber::Movable);
